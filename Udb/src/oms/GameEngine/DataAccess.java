...
static{
	oms.GameEngine.DataAccess.inputStream = null /* 0 */;
}
...
public void CloseInputFile(){
	this.inputFileLen = 0 /* 0 */;
	try {
	oms.GameEngine.DataAccess.inputStream.close();
	//end of try
...
public boolean OpenInputFile(android.content.Context context,String file){
	...
	;
	;
	oms.GameEngine.DataAccess.inputStream = new java.io.DataInputStream(new java.io.BufferedInputStream(in));
	this.inputFileLen = oms.GameEngine.DataAccess.inputStream.available();
	//end of try
...
public void read(byte[] buffer){
	try {
	oms.GameEngine.DataAccess.inputStream.read(buffer);
	//end of try
	} catch (java.io.IOException e/* */) {
		e.printStackTrace();
	} //end of catch: goto :goto_0
