...
public class PackageManager extends oms.GameEngine.PackageBase  {
		...
		if (mRes == null) {
			...
			if (fileLen != 0) {
				...
				try {
					...
					// :goto_2
				byte PackageType = (byte) (oms.GameEngine.PackageManager
						.GetPackageType(buffer) & 0xff);
				short PackageFactoryA = (short) (oms.GameEngine.PackageManager
						.GetPackageFactorA(buffer) & 65535);
				short PackageFactoryB = (short) (oms.GameEngine.PackageManager
						.GetPackageFactorB(buffer) & 65535);
				int InvalidPackageLeng = oms.GameEngine.PackageManager
				...
				bmp = android.graphics.BitmapFactory.decodeByteArray(buffer,
						...
						oms.GameEngine.C_Lib.bitmapOptions);
				buffer = null;// (byte[])0;
				;
				return bmp;
			}
