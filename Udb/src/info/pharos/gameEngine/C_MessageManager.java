...
public info.pharos.gameEngine.C_MSG GetMessageQueue(){
	if (this.nQueueLen == 0) {
	;
	return null /* 0 */;
	} else {
	info.pharos.gameEngine.C_MSG msg = (info.pharos.gameEngine.C_MSG)this.pMessageQueue.get((this.nQueueLen - 1));
	;
	return msg;
		}
	// goto :goto_0
...
public info.pharos.gameEngine.C_MSG GetMessageQueue(int queueIdx){
	if (queueIdx >= this.nQueueLen) {
	;
	return null /* 0 */;
	} else {
	info.pharos.gameEngine.C_MSG msg = (info.pharos.gameEngine.C_MSG)this.pMessageQueue.get(queueIdx);
	;
	return msg;
		}
	// goto :goto_0
...
public void SendMessage(int hWnd,int message,int param){
	;
	info.pharos.gameEngine.C_MSG msg = new info.pharos.gameEngine.C_MSG();
