...
public C_MultiTouch(){
	...
	info.pharos.gameEngine.OS.determineAPI();
	this.initTouch();
	info.pharos.gameEngine.C_MultiTouch.SetScreenOffset(0,0);
}
...
public static void SetScreenOffset(int XOff,int YOff){
	info.pharos.gameEngine.C_MultiTouch.nScreenXOff = XOff;
	info.pharos.gameEngine.C_MultiTouch.nScreenYOff = YOff;
	return;
}
...
protected void handleMultiTouch(android.view.MotionEvent event){
	int pointerCount = info.pharos.gameEngine.C_MotionEventWrapper5.getPointerCount(event);
	int action = event.getAction();
	int idx = 0;
	while (idx < pointerCount) {
	int id = info.pharos.gameEngine.C_MotionEventWrapper5.getPointerId(event,idx);
	float realX = info.pharos.gameEngine.C_MotionEventWrapper5.getX(event,idx);
	float realY = info.pharos.gameEngine.C_MotionEventWrapper5.getY(event,idx);
	int X = (int)(realX - (float)info.pharos.gameEngine.C_MultiTouch.nScreenXOff);
	int Y = (int)(realY - (float)info.pharos.gameEngine.C_MultiTouch.nScreenYOff);
...
protected void handleSDK8MultiTouch(android.view.MotionEvent event){
	int pointerCount = info.pharos.gameEngine.C_MotionEventWrapper8.getPointerCount(event);
	int action = event.getAction();
	int idx = 0;
		while (idx < pointerCount) {
			int id = info.pharos.gameEngine.C_MotionEventWrapper8.getPointerId(event,
					idx);
			float realX = info.pharos.gameEngine.C_MotionEventWrapper8.getX(event, idx);
			float realY = info.pharos.gameEngine.C_MotionEventWrapper8.getY(event, idx);
			int X = (int) (realX - (float) info.pharos.gameEngine.C_MultiTouch.nScreenXOff);
			int Y = (int) (realY - (float) info.pharos.gameEngine.C_MultiTouch.nScreenYOff);
			int x = this.ChangeToLpX(info.pharos.gameEngine.GameMath.convertToRealX(X),
					info.pharos.gameEngine.GameMath.convertToRealY(Y));
			int y = this.ChangeToLpY(info.pharos.gameEngine.GameMath.convertToRealX(X),
					info.pharos.gameEngine.GameMath.convertToRealY(Y));
			switch ((action & 0xff)) {
			case 0:
				this.onTouchDown((float) x, (float) y, pointerCount, id);
