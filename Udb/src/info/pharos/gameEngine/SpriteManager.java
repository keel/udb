...
public class SpriteManager extends Object {
	...
	// }
	// instance fields
	private info.pharos.gameEngine.ACTStruct ACTStructInfo;
	private info.pharos.gameEngine.SpriteDEF[] Sprite;
	private info.pharos.gameEngine.SpriteResDEF[] SpriteRes;
	private info.pharos.gameEngine.SpriteManager.SpriteRESACTINFO[] SpriteResACTInfo;
	private boolean bLoadingSpriteRes;
	private android.graphics.Matrix cMatrix;
	private info.pharos.gameEngine.SpriteManager.SpriteACTFILE cSpriteACTFile;
	...
	public SpriteManager(android.content.Context context, int SpriteResNum,
		...
		this.nViewRc = info.pharos.gameEngine.GameCanvas.GetViewRect();
		;
...
//		return;
	...
	private void GetFileHead(info.pharos.gameEngine.RESApp rs, int ACTId) {
		int FileHeadSize = 4;
		;
...
//		return;
	...
	public boolean CHKACTTouch(int SACTName, int SXVal, int SYVal,
			int DACTName, int DXVal, int DYVal) {
		...
		if (info.pharos.gameEngine.GameMath.CHKTouch(SXHitL, SXVal, SXHitR,
				DXHitL, DXVal, DXHitR)) {
			...
			if (info.pharos.gameEngine.GameMath.CHKTouch(SYHitU, SYVal, SYHitD,
					DYHitU, DYVal, DYHitD)) {
	...
	public boolean CHKACTTouch(int SXVal, int SYVal, int SXHitL, int SXHitR,
			int SYHitU, int SYHitD, int DACTName, int DXVal, int DYVal) {
		int DXHitL = this.GetSpriteXHitL(DACTName);
		int DXHitR = this.GetSpriteXHitR(DACTName);
		if (info.pharos.gameEngine.GameMath.CHKTouch(SXHitL, SXVal, SXHitR,
				DXHitL, DXVal, DXHitR)) {
	...
	public void FreeACT(int ACTLibIdx) {
		...
		if (this.mIsLogOut) {
			// android.util.Log.v("GameEngine",new
			// StringBuilder("Picture use RAM: ").append((this.nCurBMPRamSize /
			// 1024)).append(" KBytes").toString());
			// :cond_0
		}
		System.gc();
		return;
...
//				android.util.Log.v(
//						"GameEngine",
//						new StringBuilder("Picture use RAM: ")
//								.append((this.nCurBMPRamSize / 1024))
//								.append(" KBytes").toString());
			}
		}
		// goto :goto_0
		// :goto_0
...
public int resId(String resName,String type){
	...
	public void InitSprite(int SpriteNum) {
		...
		;
		this.Sprite = new info.pharos.gameEngine.SpriteDEF[this.nMaxSpriteNum];
		int i = 0;
		while (i < this.nMaxSpriteNum) {
			;
			this.Sprite[i] = new info.pharos.gameEngine.SpriteDEF();
			i = (i + 0x1);
			// goto :goto_0
			// :cond_1
		}
	...
	public void InitSpriteACTFile() {
		;
		this.cSpriteACTFile = new info.pharos.gameEngine.SpriteManager.SpriteACTFILE();
		return;
	}
	...
	public void InitSpriteRes(int SpriteResNum) {
		...
		;
		this.SpriteRes = new info.pharos.gameEngine.SpriteResDEF[this.nMaxSpriteResNum];
		;
		this.SpriteResACTInfo = new info.pharos.gameEngine.SpriteManager.SpriteRESACTINFO[this.nMaxSpriteResNum];
		int i = 0;
		while (i < this.nMaxSpriteResNum) {
			;
			this.SpriteRes[i] = new info.pharos.gameEngine.SpriteResDEF();
			;
			this.SpriteResACTInfo[i] = new info.pharos.gameEngine.SpriteManager.SpriteRESACTINFO();
			i = (i + 0x1);
			// goto :goto_0
			// :cond_1
		}
		;
		this.pSpriteResSegInfo = new info.pharos.gameEngine.SpriteResSeg[this.nMaxSpriteResNum];
		i = 0 /* 0 */;
		while (i < this.nMaxSpriteResNum) {
			;
			this.pSpriteResSegInfo[i] = new info.pharos.gameEngine.SpriteResSeg();
			i = (i + 0x1);
			// goto :goto_1
			// :cond_2
		}
		;
		this.ACTStructInfo = new info.pharos.gameEngine.ACTStruct();
		this.bLoadingSpriteRes = false /* 0 */;
		this.nCurSpriteFileNum = 0 /* 0 */;
		return;
	}
	...
	public void InitSpriteResInfo(int ResId, int ACTLibId, int ACTFrameBeg,
			int ACTFrameEnd) {
		if (!this.LoadSpriteACTFile(ResId)) {
			return;
		} else {
	...
	public void LoadACT(int ACTLibId, int ACTFrameBeg, int ACTFrameEnd,
			int ACTFileName) {
		ACTLibId = (ACTLibId + this.mACTLibBeg);
		if (ACTLibId >= this.nMaxSpriteResNum) {
		} else {
			this.InitSpriteResInfo(ACTFileName, ACTLibId, ACTFrameBeg,
					ACTFrameEnd);
			this.CloseSpriteACTFile();
		}
		// goto :goto_0
...
//		}
			...
			while (i < this.nShowSpriteNum) {
				...
				if (SpriteIdx != -1 && this.SpriteRes[SpriteIdx].Sprite != null
					...
					case 0:
						...
						break;
					case 1:
						SpriteXVal = ((info.pharos.gameEngine.GameCanvas
					...
					default:
						SpriteXVal = ((info.pharos.gameEngine.GameCanvas
								.GetScreenHeight() - this.Sprite[i].SpriteYVal) + XOff);
						SpriteYVal = ((info.pharos.gameEngine.GameCanvas
	...
	public boolean OnDraw(android.graphics.Canvas canvas, int Attrib,
		...
		if (this.nCurFlushSpriteNum < this.nShowSpriteNum) {
			...
			while (i < this.nShowSpriteNum) {
				...
				if (SpriteIdx != -1 && this.SpriteRes[SpriteIdx].Sprite != null
					...
					default:
						SpriteXVal = (info.pharos.gameEngine.GameCanvas
								.GetScreenHeight() - this.Sprite[i].SpriteYVal);
						SpriteYVal = (info.pharos.gameEngine.GameCanvas
								.GetScreenWidth() - this.Sprite[i].SpriteXVal);
					} // end of switch
						// :pswitch_0
					if ((SpriteYVal >= this.nViewRc.top || SpriteYVal <= this.nViewRc.bottom)
							&& (SpriteXVal >= this.nViewRc.left || SpriteXVal <= this.nViewRc.right)) {
						...
						canvas.drawBitmap(
								...
								(float) (SpriteXVal - this.Sprite[i].SpriteCenterX),
								(float) (SpriteYVal - this.Sprite[i].SpriteCenterY),
								paint);
						this.nCurFlushSpriteNum = (this.nCurFlushSpriteNum + 0x1);
						// :cond_5
					}
				...
				if (this.nCurFlushSpriteNum == this.nShowSpriteNum) {
					;
					return true /* 1 */;
					// goto somewhere; //maybe return,continue,break: goto
					// :goto_1
					// :cond_6
				}
				i = (i + 0x1);
