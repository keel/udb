...
private void ReadSpriteResInfo(int ACTFileId,int ACTLibId){
	...
	if (bmp == null) {
	bmp = info.pharos.gameEngine.PackageManager.loadPic(this.mContext,this.ACTStructInfo.ResID);
	// :cond_8
	}
	if (bmp != null) {
...
public boolean CHKACTTouch(int SACTName,int SXVal,int SYVal,int DACTName,int DXVal,int DYVal){
	...
	if (info.pharos.gameEngine.GameMath.CHKTouch(SXHitL,SXVal,SXHitR,DXHitL,DXVal,DXHitR)) {
	...
	if (info.pharos.gameEngine.GameMath.CHKTouch(SYHitU,SYVal,SYHitD,DYHitU,DYVal,DYHitD)) {
...
public int GetACTCount(int ACTId,int ACTFileName){
	...
	;
	info.pharos.gameEngine.RESApp rs = new info.pharos.gameEngine.RESApp(this.mContext);
...
public boolean OnDraw(android.graphics.Canvas canvas,int Attrib,android.graphics.Paint paint){
	...
	break;
	default:
	SpriteXVal = (info.pharos.gameEngine.GameCanvas.GetScreenHeight() - this.Sprite[i].SpriteYVal);
	SpriteYVal = (info.pharos.gameEngine.GameCanvas.GetScreenWidth() - this.Sprite[i].SpriteXVal);
	} //end of switch
	//:pswitch_0
