...
public static boolean isCollapse(int[] P1,int[] P2,int P1Num,int P2Num){
	int i = 0;
	while (i < (P1Num - 1)) {
	if (info.pharos.gameEngine.GameMath.isPointInPolygon(P1[(i << 0x1)],P1[((i << 0x1) + 0x1)],P2,P2Num)) {
	;
//	return (i << 0x1);
	return true;
	// goto :goto_2
	...
	if (info.pharos.gameEngine.GameMath.isPointInPolygon(P2[(i << 0x1)],P2[((i << 0x1) + 0x1)],P1,P1Num)) {
		;
		return true /* 1 */;
		// goto :goto_2
	} else {
...
public static boolean isPointInPolygon(int x,int y,int[] Polygon,int PolygonPointNum){
	...
	if (info.pharos.gameEngine.GameMath.isPointInLine(x,y,Polygon[(i << 0x1)],Polygon[((i << 0x1) + 0x1)],Polygon[((i << 0x1) + 0x2)],Polygon[((i << 0x1) + 0x3)])) {
